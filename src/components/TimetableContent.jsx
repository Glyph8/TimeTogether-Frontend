...
const TimetableContent = ({ isPlaceConfirmed, meetType, setMeetType }) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data) {
          console.log(
            "timetableData에서 시간표 요청 성공 유효한 데이터 로드 : ",
            res.data
          );
  ...
  return (
    <div className="timetable-content">
      ...
      <GroupTimetable
        timetableData={stableTimetableData}
        timeRange={timeRange}
      ...
      {groupCellModal ? (
        <GroupCellModal timetableData={stableTimetableData2} />
      ) : null}

      {loadPersonalTime ? (
        <div className="calender-priority-btn">
          <button
            className={`load-calender-btn ${
              isEdited === false ? "" : "editedBlock"
            }`}
            onClick={() => {
              if (!isEdited) {
                ...
                axios
                  .post(
                    ...
                    `http://${ip}:8080/group/${groupId}/when/${meetTitle}/${meetType}/load`,
                    null,
                    {
                      headers: {
                        Authorization: `Bearer ${accessToken}`,
                  ...
                  .catch((err) => {
                    console.log(
                      `TimetableContent에서 캘린더 불러오기 요청실패 ${err}`
                    );
                    // dummy 시작
                    const resData = {
                      message: "요청에 성공했습니다.",
        ...
        {loadPersonalTime ? (
          <p
            onClick={() => {
              ...
              console.log(
                `저장하기 post update요청 url /group/${groupId}/when/${meetTitle}/${meetType}/update`
              );
              console.log(
                "post요청 body : ",
              ...
              console.log(
                `요청 API http://${ip}:8080/group/${groupId}/when/${meetTitle}/${meetType}/update \n${
                  store.getState().personalTimeData
                }`
              );
              axios
                .post(
                  `http://${ip}:8080/group/${groupId}/when/${meetTitle}/${meetType}/update`,
                  store.getState().personalTimeData,
                  {
                    headers: {
                      Authorization: `Bearer ${accessToken}`,
                ...
                .catch((err) => {
                  console.log(
                    `timeTableContent에서 내 시간표 update 요청실패 ${err}`
                  );
                });
            }}
          >
          ...
          <p
            onClick={() => {
              if (!loadPersonalTime) {
                ...
                console.log(
                  `개인시간표 요청 url http://${ip}:8080/group/${groupId}/when/${meetTitle}/${meetType}/add`
                );
                axios
                  .post(
                    `http://${ip}:8080/group/${groupId}/when/${meetTitle}/${meetType}/add`,
                    null,
                    {
                      headers: {
                        Authorization: `Bearer ${accessToken}`,
                  ...
                  .catch((err) => {
                    console.log(
                      `timeTableContent에서 내 시간표 생성 요청실패 ${err}`
                    );
                  });
              }
            }}
      ...
      <div className="done-container">
        ...
        <div className="done-time-slots">
          {timeSlots.map((slot) => (
            <div
              key={slot.id}
              className={`done-time-slot ${
                selectedSlot === slot.id ? "done-selected" : ""
              }`}
        ...
        <button
          className={`done-decision-button ${
            selectedSlot && (meetType === "ONLINE" || isPlaceConfirmed)
              ? "done-selected"
              : ""
          }`}
          onClick={() => {
            ...
            if (
              selectedSlot !== null &&
              (meetType === "ONLINE" || isPlaceConfirmed)
            ) {
              //이 자리에 장소 결정완료 상태값
              ...
              console.log(
                `timeTableContent에서 Done post 요청, /group/${groupId}/when/${meetTitle}/${meetType}/done`
              );
              console.log(`post 요청 바디 : ${doneTime.date}T${startTime}`);
              // axios.post(`${testip}/group/${groupId}/when/${meetTitle}/${meetType}/done`, {
              axios
                .post(
                  `http://${ip}:8080/group/${groupId}/when/${meetTitle}/${meetType}/done`,
                  {
                ...
                .then((res) => {
                  ...
                  // setLoadPersonalTime(true); //내 시간표 나타남
                  navigate(`/meeting-list/${groupId}/`, { replace: true }); //?totalNumber=${totalNumber}&isMgr=${group.mgr}`)
                })
                .catch((err) => {
                  console.log(`timeTableContent에서 회의 결정 요청실패 ${err}`);

                  alert("회의 결정 요청에 실패하였습니다.");
                  //navigate(`/meeting-list/${groupId}/`, {replace : true})//?totalNumber=${totalNumber}&isMgr=${group.mgr}`)
                  //navigate(-1);
                  //window.location.reload()
                });
...
function returnMyTimeTable(timetableData, myUserName) {
  ...
  for (let i = 0; i < timetableData.users.length; i++) {
    // console.log('순회하며 이름 비교 ',timetableData.users[i].userName, myUserName);
    if (timetableData.users[i].userName === myUserName) {
      // if (timetableData.users[i].userId === myUserId) {
      console.log(
        "개인시간표가 있어서 해당 시간표를 로드함.",
        timetableData.users[i].days
      );
